...
# -*- coding: utf-8 -*-
"""
Created on Sat May 19 18:49:07 2018
...
1D CONV(128 filters, kernel_size = 6, strides = 6) + MAX_POOLING(4x4) + 1D CONV(128 filters, kernel_size = 6, strides = 6) + MAX_POOLING(5x5) + FLATTEN + DENSE(400) + DROPOUT(0.5) + DENSE(num_classes = 50)
...
from keras.layers import Conv2D, MaxPooling2D, Dense, Flatten, Dropout, GlobalMaxPooling2D
...
def cross_validation(X, y, tituloIm):
    ...
    for iteracion in range(6):
        ...
        #longerCNN = False    # Para hacer la red de tres convoluciones, ponerlo a True
...
if __name__ == "__main__":	
    ...
    acc_7_MFCC1 = cross_validation(X_train, y_train, "TESIS_7_MFCC1")
    ...
    acc_24_48_MFCC_h5 = cross_validation(X_train, y_train, "TESIS_24-48_MFCC_h5")
    ...
    X_train, X_test, y_train, y_test = train_test_split(Datasets["X"], Datasets["y"], test_size = 0.1, random_state=42)
    acc_12_24_MFCC_h5 = cross_validation(X_train, y_train, "TESIS_12-24_MFCC_h5")
    print("ACC 12-24 SECONDS MFCC H5-->" + str(acc_12_24_MFCC_h5))  
    
    
    Datasets = LoadDatasets("../02-Data_out/Dataset_MFCC_h5_6-12_segs.pickle")
    X_train, X_test, y_train, y_test = train_test_split(Datasets["X"], Datasets["y"], test_size = 0.1, random_state=42)
    acc_6_12_MFCC_h5 = cross_validation(X_train, y_train, "TESIS_6-12_MFCC_h5")
